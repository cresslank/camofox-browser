...
function parseWindowsProcesses(output, { strict = false } = {}) {
  try {
    const parsed = JSON.parse(String(output).trim());
    const records = (Array.isArray(parsed) ? parsed : [parsed]).map(normalizeWindowsProcess);
    if (strict && records.some(record => !record)) throw new Error('invalid Windows process record');
...
export function snapshotWindowsProcesses({ strict = false } = {}) {
