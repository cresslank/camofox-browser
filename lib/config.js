...
function loadConfig({ configPath = CONFIG_PATH } = {}) {
  ...
  const testBrowserLaunchHarness = nodeEnv === 'test' && process.env.CAMOFOX_TEST_LAUNCH_EVENTS_PATH
    ? {
        eventsPath: process.env.CAMOFOX_TEST_LAUNCH_EVENTS_PATH,
        delaysMs: String(process.env.CAMOFOX_TEST_LAUNCH_DELAYS_MS || '0')
          .split(',')
          .map(value => Math.max(0, parseInt(value.trim(), 10) || 0)),
        launchTimeoutMs: Math.max(1, parseInt(process.env.CAMOFOX_TEST_LAUNCH_TIMEOUT_MS, 10) || 25),
        warmRetryDelayMs: Math.max(1, parseInt(process.env.CAMOFOX_TEST_WARM_RETRY_DELAY_MS, 10) || 15),
      }
    : null;
  ...
  const browserPssRestartThresholdMb = parseInt(
    process.env.BROWSER_PSS_RESTART_THRESHOLD_MB || process.env.BROWSER_RSS_RESTART_THRESHOLD_MB,
    10,
  ) || 1500;
  ...
  return {
    ...
    browserPssRestartThresholdMb,
    // Compatibility property for callers written before the metric was corrected to PSS.
    browserRssRestartThresholdMb: browserPssRestartThresholdMb,
    ...
    serverEnv: {
      ...
      BROWSER_PSS_RESTART_THRESHOLD_MB: process.env.BROWSER_PSS_RESTART_THRESHOLD_MB,
      BROWSER_RSS_RESTART_THRESHOLD_MB: process.env.BROWSER_RSS_RESTART_THRESHOLD_MB,
