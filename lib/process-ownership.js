...
export function captureOwnedBrowserProcesses(rootPid, procRoot = '/proc') {
  if (process.platform === 'win32' && procRoot === '/proc') {
    try {
      const owned = selectWindowsProcessTree(rootPid, snapshotWindowsProcesses({ strict: true })).filter(isWindowsBrowserProcess);
      return {
        processes: owned.filter(proc => proc.startTime),
        indeterminate: owned.filter(proc => !proc.startTime).map(proc => ({ pid: proc.pid, error: new Error('missing Windows creation time') })),
      };
    } catch (error) {
...
export function inspectOwnedBrowserProcesses(snapshot, procRoot = '/proc') {
  if (process.platform === 'win32' && procRoot === '/proc') {
    try {
      const current = snapshotWindowsProcesses({ strict: true });
      return {
        survivors: snapshot.filter(proc => isWindowsProcessCurrent(proc, current)),
        indeterminate: snapshot.filter(proc => !proc.startTime || current.some(item => item.pid === proc.pid && !item.startTime))
          .map(proc => ({ proc, error: new Error('missing Windows creation time') })),
      };
    } catch (error) {
      return { survivors: [], indeterminate: snapshot.map(proc => ({ proc, error })) };
    }
  }
  const survivors = [];
  const indeterminate = [];
