...
/**
 * Sum proportional memory for browser descendants owned by this server process.
 * Playwright's Browser object does not expose process(), so the ownership-safe
 * fallback must walk this Node process's descendants instead of matching generic
 * Firefox process names across the host.
 */
export function browserDescendantTreePssMb(ownerPid, { procRoot = '/proc' } = {}) {
  if (process.platform !== 'linux') return null;
  if (!ownerPid || !Number.isInteger(ownerPid) || ownerPid <= 0) return null;
  try {
    const children = readChildPids(ownerPid, procRoot);
    if (children.length === 0) return null;
    return sumProcessTreeMb(children, readPssKb, procRoot);
  } catch {
    return null;
  }
}
...
export function browserProcessNameRssMb(ownerPid = null, options = {}) {
  if (!ownerPid || !Number.isInteger(ownerPid) || ownerPid <= 0) return null;
  return browserDescendantTreePssMb(ownerPid, options);
}
