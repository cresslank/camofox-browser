...
/**
 * Sum RSS for an identity-stable browser process tree. The procfs snapshot uses
 * PPID ancestry plus process start times rather than the racy task children file.
 */
export function browserProcessTreeRssMb(browserPid, { procRoot = '/proc' } = {}) {
  ...
  try {
    const capture = captureOwnedBrowserProcesses(browserPid, procRoot);
    if (capture.indeterminate.length > 0) return null;
    const ownedProcesses = capture.processes;
    if (ownedProcesses.length === 0) return null;
    return sumOwnedProcessListMb(ownedProcesses, readVmRssKb, procRoot);
  } catch {
    return null;
  }
}
...
function sumOwnedProcessListMb(processes, readMemoryKb, procRoot = '/proc') {
  ...
  for (const ownedProcess of processes) {
    try {
      ...
      // unrelated process to this server's browser. An unreadable identity
      // makes the whole metric unavailable rather than silently undercounting.
      const inspection = inspectOwnedBrowserProcesses([ownedProcess], procRoot);
      if (inspection.indeterminate.length > 0) return null;
      if (inspection.survivors.length === 0) continue;
      totalKb += memoryKb;
      found = true;
    } catch {
...
export function browserOwnedProcessPssMb(ownerPid, { procRoot = '/proc' } = {}) {
  ...
  try {
    const capture = captureOwnedBrowserProcesses(ownerPid, procRoot);
    if (capture.indeterminate.length > 0) return null;
    const ownedProcesses = capture.processes;
    if (ownedProcesses.length === 0) return null;
    return sumOwnedProcessListMb(ownedProcesses, readPssKb, procRoot);
...
export function evaluateBrowserMemoryPressure({
  ...
  if (!eligible) {
    return { action: 'skipped', consecutiveOverThreshold: 0 };
  }
